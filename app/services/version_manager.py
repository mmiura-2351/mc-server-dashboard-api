    ...
    async def _get_vanilla_versions(self) -> List[VersionInfo]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for version_data in manifest["versions"]:
                if version_data["type"] == "release":  # Only stable releases
                    ...
                    async with session.get(version_data["url"]) as version_response:
                        ...
                        if (
                            "downloads" in version_info
                            and "server" in version_info["downloads"]
                        ):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for version_id in project_info["versions"]:
                try:
                    ...
                    builds_url = f"https://api.papermc.io/v2/projects/paper/versions/{version_id}/builds"
                    ...
                    if builds_data["builds"]:
                        ...
                        latest_build = max(
                            builds_data["builds"], key=lambda x: x["build"]
                        )
                    ...
                    logger.warning(
                        f"Failed to get Paper build for version {version_id}: {e}"
                    )
                    continue
