"""SQLAlchemy implementation of the backups-domain `BackupsUnitOfWork`.

Mirrors `app.groups.adapters.uow.SqlAlchemyGroupsUnitOfWork` (see that
...
bound repositories (`SqlAlchemyBackupRepository` +
`SqlAlchemyBackupScheduleRepository`) and the attribute names
(`backups`, `schedules`).
"""
...
class SqlAlchemyBackupsUnitOfWork:
    """SQLAlchemy-backed `BackupsUnitOfWork`."""
    ...
    async def __aenter__(self) -> "SqlAlchemyBackupsUnitOfWork":
        if self._db is None:
            assert self._session_factory is not None  # for type checker
            self._db = self._session_factory()
        self.backups = SqlAlchemyBackupRepository(self._db)
        self.schedules = SqlAlchemyBackupScheduleRepository(self._db)
        self._committed = False
        return self
    ...
    async def __aexit__(
        ...
        try:
            if exc_type is not None:
                ...
                logger.warning(
                    "SqlAlchemyBackupsUnitOfWork exited with pending writes "
