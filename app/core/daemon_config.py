...
class DaemonConfig(BaseModel):
    ...
    daemon_mode: DaemonMode = Field(
        default=DaemonMode.DOUBLE_FORK,
        description="Method for creating daemon processes"
    )
    
    enable_process_persistence: bool = Field(
        default=True,
        description="Enable PID file creation and process restoration"
    )
    
    pid_file_directory: Optional[Path] = Field(
        default=None,
        description="Directory for PID files (defaults to server directory)"
    )
    
    # Process monitoring settings
    enable_process_monitoring: bool = Field(
        default=True,
        description="Enable continuous process health monitoring"
    )
    ...
    monitoring_interval_seconds: int = Field(
        ...
        ge=1,
        le=60,
        description="Interval between process health checks"
    )
    ...
    process_startup_timeout_seconds: int = Field(
        ...
        ge=5,
        le=300,
        description="Timeout for process startup verification"
    )
    ...
    log_level: LogLevel = Field(
        ...
        description="Logging level for daemon operations"
    )
    
    enable_daemon_logs: bool = Field(
        default=True,
        description="Enable logging for daemon process operations"
    )
    ...
    enable_process_isolation: bool = Field(
        default=True,
        description="Enable process isolation verification"
    )
    
    verify_detachment: bool = Field(
        default=True,
        description="Verify proper daemon detachment"
    )
    
    secure_environment: bool = Field(
        default=True,
        description="Use secure environment variables"
    )
    
    # RCON integration settings
    enable_rcon_integration: bool = Field(
        default=True,
        description="Enable RCON for real-time commands"
    )
    ...
    rcon_timeout_seconds: int = Field(
        ...
        ge=1,
        le=60,
        description="RCON connection timeout"
    )
    ...
    rcon_retry_attempts: int = Field(
        ...
        ge=1,
        le=10,
        description="Number of RCON retry attempts"
    )
    
    # Process recovery settings
    enable_auto_recovery: bool = Field(
        default=True,
        description="Enable automatic process recovery on startup"
    )
    ...
    recovery_timeout_seconds: int = Field(
        ...
        ge=10,
        le=600,
        description="Timeout for recovery operations"
    )
    ...
    max_recovery_attempts: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum recovery attempts per process"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pid_file_directory": str(self.pid_file_directory) if self.pid_file_directory else None,
    ...
    def from_environment(cls) -> 'DaemonConfig':
        ...
        for env_var, config_key in env_mapping.items():
            ...
            if value is not None:
                ...
                if config_key in ['enable_process_persistence', 'enable_process_monitoring', 
                                'enable_daemon_logs', 'enable_process_isolation', 
                                'verify_detachment', 'secure_environment', 
                                'enable_rcon_integration', 'enable_auto_recovery']:
                    config_data[config_key] = value.lower() in ('true', '1', 'yes', 'on')
                elif config_key in ['monitoring_interval_seconds', 'process_startup_timeout_seconds',
                                  'log_rotation_size_mb', 'rcon_timeout_seconds', 
                                  'rcon_retry_attempts', 'recovery_timeout_seconds',
