...
Implements `app.versions.domain.ports.VersionRepository`.
"""
...
class SqlAlchemyVersionRepository:
    ...
    replacing slow external API calls.
    """
    ...
    async def deactivate_versions(
        ...
        """Deactivate versions not in the keep list; returns number deactivated."""
    ...
    async def cleanup_old_versions(self, days_old: int = 30) -> int:
        """Remove very old inactive versions; returns number deleted."""
    ...
    async def update_log_counts(
        ...
        """Persist the per-type counts on an update log entry."""
        self.db.query(VersionUpdateLog).filter_by(id=log_id).update(
            {
    ...
    def delete_version(self, version_id: int) -> bool:
        """Delete a version by ID; returns True if deleted, False if not found."""
        version = (
            self.db.query(MinecraftVersion)
            .filter(MinecraftVersion.id == version_id)
            .first()
