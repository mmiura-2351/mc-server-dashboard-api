"""SQLAlchemy implementation of the templates-domain `TemplatesUnitOfWork`.

Mirrors `app.files.adapters.uow.SqlAlchemyFilesUnitOfWork` (see that
...
bound repository (`SqlAlchemyTemplateRepository`) and the attribute
name (`templates`).
"""
...
class SqlAlchemyTemplatesUnitOfWork:
    """SQLAlchemy-backed `TemplatesUnitOfWork`."""
    ...
    async def __aenter__(self) -> "SqlAlchemyTemplatesUnitOfWork":
        if self._db is None:
            assert self._session_factory is not None  # for type checker
            self._db = self._session_factory()
        self.templates = SqlAlchemyTemplateRepository(self._db)
        self._committed = False
        return self
    ...
    async def __aexit__(
        ...
        try:
            if exc_type is not None:
                ...
                logger.warning(
                    "SqlAlchemyTemplatesUnitOfWork exited with pending writes "
