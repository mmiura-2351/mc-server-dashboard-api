import pytest
from fastapi import status
from fastapi.testclient import TestClient

from app.servers.models import Server, ServerStatus, ServerType
from app.users.models import Role


class TestServerPortConflicts:
    
    def test_create_server_port_conflict_with_running_server(self, client: TestClient, admin_headers, db, admin_user):
        """Test that creating a server fails when port conflicts with running server"""
        # Create a running server with port 25565
        running_server = Server(
            name="Running Server",
            description="A running server",
            minecraft_version="1.20.1",
            server_type=ServerType.vanilla,
            status=ServerStatus.running,
            directory_path="./servers/running_server",
            port=25565,
            max_memory=1024,
            max_players=20,
            owner_id=admin_user.id
        )
        db.add(running_server)
        db.commit()
        
        # Try to create another server with the same port
        server_data = {
            "name": "Conflicting Server",
            "description": "This should fail",
            "minecraft_version": "1.20.1",
            "server_type": "vanilla",
            "port": 25565,
            "max_memory": 1024,
            "max_players": 20
        }
        
        response = client.post("/api/v1/servers/", headers=admin_headers, json=server_data)
        
        assert response.status_code == status.HTTP_409_CONFLICT
        assert "already running" in response.json()["detail"]
    
    def test_create_server_port_conflict_with_stopped_server_allowed(self, client: TestClient, admin_headers, db, admin_user):
        """Test that creating a server succeeds when port conflicts with stopped server"""
        # Create a stopped server with port 25566
        stopped_server = Server(
            name="Stopped Server",
            description="A stopped server", 
            minecraft_version="1.20.1",
            server_type=ServerType.vanilla,
            status=ServerStatus.stopped,
            directory_path="./servers/stopped_server",
            port=25566,
            max_memory=1024,
            max_players=20,
            owner_id=admin_user.id
        )
        db.add(stopped_server)
        db.commit()
        
        # Try to create another server with the same port as stopped server
        server_data = {
            "name": "New Server",
            "description": "This should succeed",
            "minecraft_version": "1.20.1", 
            "server_type": "vanilla",
            "port": 25566,
            "max_memory": 1024,
            "max_players": 20
        }
        
        response = client.post("/api/v1/servers/", headers=admin_headers, json=server_data)
        
        assert response.status_code == status.HTTP_201_CREATED
        server_response = response.json()
        assert server_response["port"] == 25566
        assert server_response["name"] == "New Server"
        
        # Cleanup
        import shutil
        from pathlib import Path
        server_dir = Path(server_response["directory_path"])
        if server_dir.exists():
            shutil.rmtree(server_dir)
    
    def test_create_server_port_conflict_with_starting_server(self, client: TestClient, admin_headers, db, admin_user):
        """Test that creating a server fails when port conflicts with starting server"""
        # Create a starting server with port 25567
        starting_server = Server(
            name="Starting Server",
            description="A starting server",
            minecraft_version="1.20.1",
            server_type=ServerType.vanilla,
            status=ServerStatus.starting,
            directory_path="./servers/starting_server", 
            port=25567,
            max_memory=1024,
            max_players=20,
            owner_id=admin_user.id
        )
        db.add(starting_server)
        db.commit()
        
        # Try to create another server with the same port
        server_data = {
            "name": "Conflicting Server",
            "description": "This should fail",
            "minecraft_version": "1.20.1",
            "server_type": "vanilla",
            "port": 25567,
            "max_memory": 1024,
            "max_players": 20
        }
        
        response = client.post("/api/v1/servers/", headers=admin_headers, json=server_data)
        
        assert response.status_code == status.HTTP_409_CONFLICT
        assert "already running" in response.json()["detail"]