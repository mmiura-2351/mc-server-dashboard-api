...
class MockJavaCompatibilityServiceFullWorkflow:
    ...
    def __init__(self):
        self.java_version = JavaVersionInfo(
            17, 0, 1, "OpenJDK", "17.0.1+12", "/usr/bin/java"
        )
...
""")
    ...
    @pytest.mark.asyncio
    async def test_server_startup_complete_workflow_success(
        self,
        manager,
        lifecycle_server,
        mock_db_session,
        ...
        async def mock_create_subprocess(*args, **kwargs):
            ...
            if args and "java" in str(args[0]):
                return await original_create_subprocess(
                    *long_running_process_command, **kwargs
                )
                ...
                with patch("app.services.minecraft_server.logger") as mock_logger:
                    ...
                    with patch.object(manager, "_create_daemon_process") as mock_daemon:
                        ...
                        with patch.object(manager, "_is_process_running") as mock_running:
                            ...
                            assert server_process.status in [
                    ...
                    startup_logs = [
                        log
                        for log in logger_calls
                        if "Starting pre-flight checks" in log
                        or "Port validation passed" in log
                        or "Java compatibility verified" in log
                    ]
            ...
            with patch(
                ...
                with patch.object(
                    ...
                    with patch("app.services.minecraft_server.logger") as mock_logger:
                        ...
                        port_error_logs = [
                            log for log in error_calls if "Port validation failed" in log
                        ]
    ...
    async def test_server_startup_java_compatibility_failure_workflow(
        ...
        with patch(
            ...
            with patch("app.services.minecraft_server.logger") as mock_logger:
                ...
                java_error_logs = [
                    log for log in error_calls if "Java compatibility check failed" in log
                ]
    ...
    async def test_server_startup_file_validation_failure_workflow(
        ...
        with patch(
            ...
            with patch("app.services.minecraft_server.logger") as mock_logger:
                ...
                error_calls = [call[0][0] for call in mock_logger.error.call_args_list]
                file_error_logs = [
                    log for log in error_calls if "file validation failed" in log
                ]
            ...
            with patch("os.fork", side_effect=OSError("Permission denied")):
                with patch("app.services.minecraft_server.logger") as mock_logger:
                    ...
                    daemon_error_logs = [
                        ...
                        if "daemon creation failed" in log
                        or "All daemon creation methods failed" in log
                    ]
                ...
                with patch.object(
                    ...
                    with patch("app.services.minecraft_server.logger") as mock_logger:
                        ...
                        daemon_death_logs = [
                            log
                            for log in error_calls
                            if ("died" in log or "not running" in log or "ended" in log)
                        ]
    ...
    @pytest.mark.asyncio
    async def test_server_shutdown_graceful_success_workflow(
        self, manager, long_running_process_command
    ):
        ...
        with patch("app.services.minecraft_server.logger") as mock_logger:
            ...
            shutdown_logs = [
                log for log in info_calls if "Successfully stopped server" in log
            ]
    ...
    async def test_server_shutdown_already_terminated_workflow(self, manager):
        ...
        with patch.object(manager, "_cleanup_server_process") as mock_cleanup:
            with patch("app.services.minecraft_server.logger") as mock_logger:
                ...
                info_calls = [call[0][0] for call in mock_logger.info.call_args_list]
                terminated_logs = [
                    log for log in info_calls if "process already terminated" in log
                ]
    ...
    async def test_server_shutdown_graceful_timeout_then_force(self, manager):
        ...
        with patch("app.services.minecraft_server.logger") as mock_logger:
            ...
            timeout_warnings = [
                ...
                if "graceful stop failed" in log and "forcing termination" in log
            ]
    ...
    @pytest.mark.asyncio
    async def test_server_shutdown_force_immediate(
        self, manager, long_running_process_command
    ):
    ...
    async def test_server_shutdown_exception_handling_workflow(self, manager):
        ...
        with patch.object(manager, "_cleanup_server_process") as mock_cleanup:
            with patch("app.services.minecraft_server.logger") as mock_logger:
                ...
                error_calls = [call[0][0] for call in mock_logger.error.call_args_list]
                stop_error_logs = [
                    log for log in error_calls if "Failed to stop server" in log
                ]
...
""")
    ...
    async def test_send_command_exception_workflow(self, manager):
        ...
        with patch("app.services.minecraft_server.logger") as mock_logger:
            ...
            error_calls = [call[0][0] for call in mock_logger.error.call_args_list]
            command_error_logs = [
                log for log in error_calls if "Failed to send command" in log
            ]
