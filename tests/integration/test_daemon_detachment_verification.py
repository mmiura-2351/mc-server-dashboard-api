...
class TestDaemonDetachmentVerification:
    ...
    @pytest.mark.asyncio
    async def test_daemon_process_detachment_verification(
        self, manager, detachment_test_server
    ):
        ...
        with patch.object(manager, "_create_daemon_process_alternative") as mock_create:
            ...
            with patch.object(manager, "_is_process_running") as mock_running:
                ...
                result = await manager._create_daemon_process_alternative(
                    cmd, cwd, env, detachment_test_server.id
                )
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_daemon_detachment_verification(
        self, manager, detachment_test_server
    ):
    ...
    async def test_daemon_detachment_verification_with_process_cleanup(self, manager):
        ...
        with patch.object(
            manager,
            "_read_pid_file",
            return_value={"pid": 555555, "server_id": test_server_id},
        ):
            with patch.object(manager, "_is_process_running", return_value=False):
                with patch.object(manager, "_remove_pid_file") as mock_remove:
                    ...
                    result = await manager._restore_process_from_pid(
                        test_server_id, test_server_dir
                    )
