...
""")
    ...
    async def test_daemon_monitoring_complete_lifecycle(self, manager):
        ...
        # Mock daemon process monitoring
        with patch.object(manager, '_monitor_daemon_process') as mock_monitor:
            ...
            with patch.object(manager, '_cleanup_server_process') as mock_cleanup:
                with patch("app.services.minecraft_server.logger") as mock_logger:
                    ...
                    # Create monitoring task to test daemon monitoring
    ...
    async def test_daemon_cleanup_complete_lifecycle(self, manager, tmp_path):
        ...
        with patch("app.services.minecraft_server.logger") as mock_logger:
            ...
            # Call notify status change manually since cleanup doesn't do it
            ...
            # Verify status change to stopped was recorded
            assert (1, ServerStatus.stopped) in status_changes
            ...
            cleanup_logs = [log for log in debug_calls if "Cleaned up resources for server 1" in log]
