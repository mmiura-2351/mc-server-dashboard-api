...
class TestDaemonLifecycleComprehensive:
    ...
    def mock_java_service(self):
        ...
        mock.validate_java_requirements = AsyncMock(
            return_value=(True, "Java 17 compatible")
        )
        return mock
    ...
    async def test_daemon_creation_complete_lifecycle(
        ...
        with patch(
            ...
            with patch.object(
                ...
                return_value=(True, "Port 25565 available"),
            ):
                with patch.object(
                    manager,
                    "_check_java_compatibility",
                    return_value=(True, "Java 17 compatible", "/usr/bin/java"),
                ):
                    with patch.object(
                        manager,
                        "_validate_server_files",
                        return_value=(True, "All files valid"),
                    ):
                        with patch.object(
                            ...
                            with patch.object(
                                manager,
                                "_ensure_rcon_configured",
                                return_value=(True, 25575, "password123"),
                            ):
                                with patch.object(
                                    manager,
                                    "_create_daemon_process_alternative",
                                    return_value=12345,
                                ):
                                    with patch.object(
                                        ...
                                        with patch(
                                            ...
                                            assert (
                                                daemon_test_server.id in manager.processes
                                            )
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    manager, "_create_daemon_process", return_value=None
                                ):
                                    with patch.object(
                                        manager,
                                        "_create_daemon_process_alternative",
                                        ...
                                        assert (
                                            daemon_test_server.id not in manager.processes
                                        )
    ...
    async def test_daemon_monitoring_lifecycle(self, manager):
        ...
        with patch.object(manager, "_is_process_running") as mock_running:
            ...
            mock_running.side_effect = [
                ...
                True,
                False,
            ]  # Running 3 times, then stops

            with patch.object(
                manager, "_cleanup_server_process", new_callable=AsyncMock
            ) as mock_cleanup:
                with patch("asyncio.sleep", new_callable=AsyncMock):
                    # Start monitoring
                    monitor_task = asyncio.create_task(
                        manager._monitor_daemon_process(server_process)
                    )
