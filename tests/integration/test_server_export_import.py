...
class TestServerExportImport:
    def test_export_server_success(
        ...
        try:
            response = client.get(
                f"/api/v1/servers/{server_id}/export", headers=admin_headers
            )
    ...
    def test_export_server_directory_not_found(
        ...
        response = client.get(
            f"/api/v1/servers/{sample_server.id}/export", headers=admin_headers
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_import_server_authorized_for_regular_user(
        ...
        response = client.post(
            "/api/v1/servers/import", headers=user_headers, files=files, data=data
        )
        assert response.status_code != status.HTTP_403_FORBIDDEN, (
            "Regular users should be allowed to import servers in Phase 1"
        )
    ...
    def test_import_server_invalid_zip(self, client: TestClient, admin_headers):
        ...
        files = {
            "file": ("test.zip", io.BytesIO(b"corrupted zip data"), "application/zip")
        }
        data = {"name": "Test Server"}
    ...
    def test_export_excludes_log_files(
        self, client: TestClient, admin_headers, sample_server
    ):
