...
""")
    ...
    @pytest.mark.asyncio
    async def test_java_compatibility_no_installations_found(
        self, manager, integration_server, mock_java_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_java_compatibility_incompatible_version(
        self, manager, integration_server, mock_java_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_java_compatibility_success_with_logging(
        self, manager, integration_server, mock_java_service
    ):
        ...
        mock_java_service.set_compatibility_result(
            True, "Java 17 is compatible with Minecraft 1.20.1"
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_server_files_jar_not_readable(
    ...
    @pytest.mark.asyncio
    async def test_validate_server_files_directory_not_writable(
        self, manager, integration_server
    ):
    ...
    async def test_validate_port_availability_database_conflict(
        ...
        assert (
            f"Port {integration_server.port} is already in use by running server 'existing-server'"
            in message
        )
    ...
    async def test_validate_port_availability_system_conflict(
        ...
        try:
            ...
            assert available is False
            assert (
                f"Port {integration_server.port} is already in use by another process"
                in message
            )
        finally:
    ...
    async def test_concurrent_server_operations(self, manager, tmp_path):
        ...
        for server in servers:
            ...
            task = asyncio.create_task(
                manager._validate_server_files(Path(server.directory_path))
            )
            tasks.append(task)
