...
class TestFileHistoryServiceEnhancedCoverage:
    ...
    def mock_history_record(self):
        ...
        record.backup_file_path = "./file_history/1/server.properties/v001_20250611_120000.properties"
    ...
    async def test_create_version_backup_success(self, service, mock_db):
        ...
        with patch.object(service, '_normalize_file_path', return_value="normalized/path.txt"):
                    ...
                    with patch.object(service, '_get_next_version_number', return_value=1):
                        ...
                        with patch('aiofiles.open', return_value=mock_file):
                            ...
                            with patch.object(service, '_cleanup_excess_versions'):
                                ...
                                with patch.object(service, '_to_record_schema', return_value=mock_record):
    ...
    async def test_create_version_backup_exception_handling(self, service, mock_db):
        ...
        with patch.object(service, '_normalize_file_path', side_effect=Exception("Normalization failed")):
            with pytest.raises(FileOperationException, match="Failed to backup file test.txt"):
    ...
    async def test_create_version_backup_file_write_error(self, service, mock_db):
        ...
        with patch.object(service, '_normalize_file_path', return_value="test.txt"):
            with patch('pathlib.Path.mkdir'):
                with patch.object(service, '_is_duplicate_content', return_value=False):
                    with patch.object(service, '_get_next_version_number', return_value=1):
                        with patch('aiofiles.open', side_effect=PermissionError("Write permission denied")):
    ...
    async def test_get_file_history_success(self, service, mock_db, mock_history_record):
        ...
        with patch.object(service, '_normalize_file_path', return_value="server.properties"):
    ...
    async def test_get_file_history_empty_result(self, service, mock_db):
        ...
        with patch.object(service, '_normalize_file_path', return_value="nonexistent.txt"):
            result = await service.get_file_history(
                server_id=1,
    ...
    @pytest.mark.asyncio
    async def test_get_version_content_success(self, service, mock_db, mock_history_record):
    ...
    async def test_get_version_content_not_found(self, service, mock_db):
        ...
        with patch.object(service, '_normalize_file_path', return_value="server.properties"):
            with pytest.raises(InvalidRequestException, match="Version 1 not found for file server.properties"):
    ...
    @pytest.mark.asyncio
    async def test_get_version_content_backup_file_not_found(self, service, mock_db, mock_history_record):
    ...
    @pytest.mark.asyncio
    async def test_get_version_content_read_error(self, service, mock_db, mock_history_record):
        ...
        with patch.object(service, '_normalize_file_path', return_value="server.properties"):
            with patch('pathlib.Path.exists', return_value=True):
                with patch('aiofiles.open', side_effect=PermissionError("Read permission denied")):
                    with pytest.raises(FileOperationException):
                        await service.get_version_content(
                            server_id=1,
                            file_path="server.properties",
    ...
    @pytest.mark.asyncio
    async def test_restore_from_history_success_with_backup(self, service, mock_db, mock_server):
        """Test successful restore with backup creation"""
        # Mock get_version_content
        with patch.object(service, 'get_version_content', return_value=("restored content", Mock())):
            # Mock server query
    ...
    @pytest.mark.asyncio
    async def test_restore_from_history_no_backup_needed(self, service, mock_db, mock_server):
    ...
    async def test_restore_from_history_server_not_found(self, service, mock_db):
        ...
        with patch.object(service, 'get_version_content', return_value=("content", Mock())):
    ...
    @pytest.mark.asyncio
    async def test_restore_from_history_backup_creation_fails(self, service, mock_db, mock_server):
        ...
        with patch.object(service, 'get_version_content', return_value=("restored content", Mock())):
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with patch.object(service, 'create_version_backup', side_effect=Exception("Backup failed")):
                    mock_file = AsyncMock()
                    mock_file.write = AsyncMock()
                    mock_file.read = AsyncMock(return_value="current content")
                    mock_file.__aenter__ = AsyncMock(return_value=mock_file)
    ...
    @pytest.mark.asyncio
    async def test_delete_version_file_not_exists(self, service, mock_db, mock_history_record):
...
class TestFileHistoryServiceIntegration:
    ...
    async def test_complete_file_lifecycle(self, service):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_db.query.return_value.filter.return_value.scalar.return_value = None  # First version
            mock_db.add = Mock()
            mock_db.commit = Mock()
    ...
    async def test_multiple_versions_scenario(self, service):
        ...
        mock_db.query.return_value.filter.return_value.scalar.side_effect = version_numbers
                ...
                with patch('pathlib.Path.mkdir'):
                    ...
                    with patch('aiofiles.open', return_value=mock_file):
                        with patch.object(service, '_to_record_schema') as mock_schema:
                            ...
                            v1 = await service.create_version_backup(1, "test.txt", "content1", 1, "v1", mock_db)
                            v2 = await service.create_version_backup(1, "test.txt", "content2", 1, "v2", mock_db)
                            v3 = await service.create_version_backup(1, "test.txt", "content3", 1, "v3", mock_db)
