...
class TestTemplateServiceEnhancedCoverage:
    ...
    @pytest.mark.asyncio
    async def test_create_template_from_server_exception_with_rollback(self, service, mock_server, mock_admin_user, mock_db):
        ...
        with patch('pathlib.Path.exists', return_value=True):  # Mock server dir exists
            with patch.object(service, '_create_template_files', side_effect=Exception("File creation failed")):
                with patch('app.services.template_service.logger') as mock_logger:
                    with pytest.raises(TemplateCreationError, match="Failed to create template: File creation failed"):
    ...
    @pytest.mark.asyncio
    async def test_create_custom_template_exception_with_rollback(self, service, mock_admin_user, mock_db):
        """Test exception handling with database rollback in create_custom_template"""
        # Mock Template constructor to raise exception
        with patch('app.services.template_service.Template', side_effect=Exception("Template creation failed")):
            with patch('app.services.template_service.logger') as mock_logger:
                with pytest.raises(TemplateCreationError, match="Failed to create template: Template creation failed"):
    ...
    async def test_extract_server_configuration_success(self, service, mock_server):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, '_parse_server_properties', return_value={"motd": "Test Server", "difficulty": "normal"}):
    ...
    @pytest.mark.asyncio
    async def test_extract_server_configuration_no_properties_file(self, service, mock_server):
    ...
    async def test_create_template_files_success(self, service):
        ...
        with tempfile.TemporaryDirectory() as server_temp_dir:
            ...
            with patch.object(service, 'templates_directory', Path(server_temp_dir) / "templates"):
    ...
    @pytest.mark.asyncio
    async def test_apply_template_to_server_success(self, service, mock_template, mock_db):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, 'templates_directory', Path(temp_dir)):
                with patch.object(service, '_apply_server_properties') as mock_apply_props:
                    result = await service.apply_template_to_server(template_id, server_dir, mock_db)
                    
                    assert result is True
                    mock_apply_props.assert_called_once()

    @pytest.mark.asyncio
    async def test_apply_template_to_server_missing_archive(self, service, mock_template, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_apply_template_to_server_tar_extraction_error(self, service, mock_template, mock_db):
    ...
    async def test_apply_server_properties_success(self, service):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(service, '_parse_server_properties', return_value={"motd": "Old MOTD", "difficulty": "easy"}):
    ...
    def test_get_template_access_denied(self, service, mock_regular_user, mock_db, mock_template):
    ...
    def test_update_template_access_denied(self, service, mock_regular_user, mock_db, mock_template):
    ...
    def test_delete_template_access_denied(self, service, mock_regular_user, mock_db, mock_template):
    ...
    def test_delete_template_file_deletion_error(self, service, mock_admin_user, mock_db, mock_template):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('pathlib.Path.unlink', side_effect=OSError("Permission denied")):
                with patch('app.services.template_service.logger') as mock_logger:
                    ...
                    with pytest.raises(TemplateError, match="Failed to delete template: Permission denied"):
    ...
    @pytest.mark.asyncio
    async def test_clone_template_success(self, service, mock_admin_user, mock_db, mock_template):
    ...
    async def test_clone_template_not_found(self, service, mock_admin_user, mock_db):
        ...
        with pytest.raises(TemplateError):  # TemplateNotFoundError gets wrapped in TemplateError
    ...
    @pytest.mark.asyncio
    async def test_clone_template_access_denied(self, service, mock_regular_user, mock_db, mock_template):
    ...
    @pytest.mark.asyncio
    async def test_clone_template_creation_error(self, service, mock_admin_user, mock_db, mock_template):
        ...
        with patch('app.services.template_service.Template', side_effect=Exception("Creation failed")):
            with pytest.raises(TemplateError):  # Should raise TemplateError, not TemplateCreationError
                await service.clone_template(
                    original_template_id=1,
                    name="Cloned Template",
                    user=mock_admin_user,
