...
class TestAuthorizationServiceBackupAccess:
    ...
    def test_check_backup_access_admin_user(self, db: Session, admin_user, sample_backup):
        ...
        result = AuthorizationService.check_backup_access(
            sample_backup.id, admin_user, db
        )
        assert result == sample_backup
        assert result.id == sample_backup.id

    def test_check_backup_access_server_owner(
...
class TestAuthorizationServiceRoleDecorators:
    ...
    async def test_require_admin_or_operator_regular_user(self, test_user):
        ...
        assert exc_info.value.status_code == status.HTTP_403_FORBIDDEN
        assert "Only operators and admins can perform this action" in str(
...
class TestAuthorizationServiceServerFiltering:
    ...
    def test_filter_servers_for_regular_user_with_db(
        self, db: Session, test_user, sample_server
    ):
    ...
    def test_filter_servers_for_operator_with_db(
        ...
        filtered = AuthorizationService.filter_servers_for_user(
...
class TestAuthorizationServiceEdgeCases:
    ...
    def test_filter_servers_with_none_db(self, test_user):
        ...
        servers = []
        with pytest.raises(
            ValueError, match="Database session is required for security filtering"
        ):
...
class TestAuthorizationServiceRoleHierarchy:
    ...
    def test_role_hierarchy_admin_highest(self, admin_user, operator_user, test_user):
        ...
        assert (
            AuthorizationService.can_create_server(admin_user)
            == AuthorizationService.can_create_server(operator_user)
            == AuthorizationService.can_create_server(test_user)
            is True
        )
        assert (
            AuthorizationService.can_modify_files(admin_user)
            == AuthorizationService.can_modify_files(operator_user)
            == AuthorizationService.can_modify_files(test_user)
            is True
        )
        assert (
            AuthorizationService.can_restore_backup(admin_user)
            == AuthorizationService.can_restore_backup(operator_user)
            == AuthorizationService.can_restore_backup(test_user)
            is True
        )
        assert (
            AuthorizationService.can_create_backup(admin_user)
            == AuthorizationService.can_create_backup(operator_user)
            == AuthorizationService.can_create_backup(test_user)
            is True
        )
        assert (
            AuthorizationService.can_create_group(admin_user)
            == AuthorizationService.can_create_group(operator_user)
            == AuthorizationService.can_create_group(test_user)
            is True
        )
        assert (
            AuthorizationService.can_create_template(admin_user)
            == AuthorizationService.can_create_template(operator_user)
            == AuthorizationService.can_create_template(test_user)
            is True
        )
...
class TestAuthorizationServiceIntegration:
    ...
    def test_server_ownership_transfer_scenario(
        self, db: Session, admin_user, test_user, operator_user
    ):
    ...
    def test_multi_server_filtering_scenario(
        self, db: Session, admin_user, test_user, operator_user
    ):
