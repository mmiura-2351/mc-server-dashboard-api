...
class TestMinecraftVersionManagerMissingCoverage:
    ...
    async def test_get_supported_versions_forge_type(self, manager):
        ...
        with patch.object(manager, '_get_forge_versions') as mock_forge:
            forge_versions = [
                VersionInfo("1.20.1", ServerType.forge, "http://forge.url", is_stable=True),
                VersionInfo("1.19.4", ServerType.forge, "http://forge.url2", is_stable=True)
            ]
    ...
    async def test_get_supported_versions_unsupported_type(self, manager):
        ...
        with patch.object(manager, '_get_fallback_versions') as mock_fallback:
            fallback_versions = [VersionInfo("1.20.1", ServerType.vanilla, "fallback_url")]
    ...
    async def test_get_download_url_version_not_found(self, manager):
        ...
        with patch.object(manager, 'get_supported_versions') as mock_get_versions:
            available_versions = [
                VersionInfo("1.20.1", ServerType.vanilla, "http://example.com/1.20.1.jar"),
                VersionInfo("1.19.4", ServerType.vanilla, "http://example.com/1.19.4.jar")
            ]
    ...
    def test_is_version_supported_exception_handling(self, manager):
        ...
        with patch.object(manager, '_is_version_supported', side_effect=Exception("Version error")):
    ...
    async def test_get_vanilla_versions_exception_in_parallel_processing(self, manager):
        ...
        mock_response = MockAiohttpResponse(
            status=200,
            json_data={"versions": [{"id": "1.20.1", "type": "release", "url": "http://test.url"}]}
        )
    ...
    async def test_fetch_vanilla_version_info_no_server_download(self, manager):
        ...
        version_data = {"id": "1.20.1", "url": "http://test.url", "releaseTime": "2023-01-01T00:00:00Z"}
        
        # Mock version response without server downloads
        mock_response = MockAiohttpResponse(
            status=200,
            json_data={"downloads": {"client": {"url": "http://client.jar"}}}
        )
        
        mock_session = MockAiohttpSession({
            "http://test.url": mock_response
        })
    ...
    async def test_get_paper_versions_exception_in_parallel_processing(self, manager):
        ...
        mock_response = MockAiohttpResponse(
            status=200,
            json_data={"versions": ["1.20.1", "1.19.4"]}
        )
        ...
        with patch('aiohttp.ClientSession', return_value=mock_session):
            with patch('asyncio.gather', new_callable=AsyncMock) as mock_gather:
                mock_gather.return_value = [Exception("Network error"), Exception("Another error")]
