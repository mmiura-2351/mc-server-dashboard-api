...
class TestPhase1ComplianceResourceAccess:
    ...
    def test_users_can_view_servers_with_visibility_system(self, db: Session, test_user, admin_user):
        ...
        assert has_access is False, "Without visibility config, access should be denied (Phase 2 secure default)"


class TestPhase1ComplianceOperationalRequirements:
    """Test that Phase 1 operational requirements are met"""

    def test_phase1_transforms_from_ownership_based_to_shared_model(self, test_user, admin_user):
    ...
    def test_view_servers_requirement(self, db: Session, test_user):
        ...
        filtered_servers = AuthorizationService.filter_servers_for_user(test_user, servers, db)
        ...
        assert isinstance(filtered_servers, list), "filter_servers_for_user should return a list"
...
class TestPhase1ComplianceIntegration:
    ...
    def test_phase1_maintains_security_boundaries(self, admin_user, operator_user, test_user):
...
class TestPhase1ComplianceWithPhase2Integration:
    ...
    def test_phase1_and_phase2_integration(self, db: Session, phase1_user, phase1_server):
        ...
        try:
            result = AuthorizationService.check_server_access(phase1_server.id, phase1_user, db)
