...
class TestConnectionManagerFixed:
    ...
    @pytest.mark.asyncio
    async def test_connect_user_success(
    ...
    @pytest.mark.asyncio
    async def test_connect_multiple_users_same_server(
        self, connection_manager, mock_user
    ):
    ...
    async def test_broadcast_server_notification(self, connection_manager):
        ...
        with patch.object(connection_manager, "send_to_server_connections") as mock_send:
            await connection_manager.broadcast_server_notification(
    ...
    def test_determine_log_type_error_messages(self, connection_manager):
        ...
        assert (
            connection_manager._determine_log_type("[ERROR] Something went wrong")
            == "error"
        )
    ...
    def test_determine_log_type_warning_messages(self, connection_manager):
        ...
        assert (
            connection_manager._determine_log_type("[WARN] Deprecated feature")
            == "warning"
        )
    ...
    def test_determine_log_type_player_events(self, connection_manager):
        ...
        assert (
            connection_manager._determine_log_type("Player123 joined the game")
            == "player_join"
        )
        assert (
            connection_manager._determine_log_type("Player123 left the game")
            == "player_leave"
        )
