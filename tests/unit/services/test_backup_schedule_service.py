...
class TestBackupScheduleService:
    ...
    def test_create_schedule_success(self, db: Session, test_server: Server):
        ...
        created_schedule = (
            ...
            .filter(BackupSchedule.server_id == test_server.id)
            .first()
        )
    ...
    def test_get_schedule_by_server_id(self, db: Session, test_server: Server):
        ...
        # Create schedule
        schedule = BackupSchedule(
            server_id=test_server.id, interval_hours=12, max_backups=10
        )
        db.add(schedule)
        db.commit()
    ...
    def test_update_schedule_success(self, db: Session, test_server: Server):
        ...
        # Create schedule
        schedule = BackupSchedule(
            server_id=test_server.id, interval_hours=12, max_backups=10, enabled=True
        )
        db.add(schedule)
        db.commit()
        ...
        # Verify
        updated_schedule = (
            db.query(BackupSchedule).filter(BackupSchedule.id == schedule.id).first()
        )
    ...
    def test_delete_schedule_success(self, db: Session, test_server: Server):
        ...
        # Create schedule
        schedule = BackupSchedule(
            server_id=test_server.id, interval_hours=8, max_backups=12
        )
        db.add(schedule)
        db.commit()
    ...
    def test_list_all_schedules(self, db: Session, test_user: User):
        ...
        enabled_schedules = (
            db.query(BackupSchedule).filter(BackupSchedule.enabled == True).all()
        )
    ...
    def test_list_schedules_with_server_relationship(self, db: Session, test_user: User):
        ...
        schedule = BackupSchedule(server_id=server.id, interval_hours=4, max_backups=20)
        db.add(schedule)
        db.commit()

        # Get with relationships
        schedule_with_server = (
    ...
    def test_schedule_execution_time_management(self, db: Session, test_server: Server):
        ...
        # Create schedule
        schedule = BackupSchedule(
            server_id=test_server.id, interval_hours=6, max_backups=10
        )
        db.add(schedule)
        db.commit()
    ...
    def test_schedule_due_for_backup_query(self, db: Session, test_user: User):
        ...
        servers_data = [
            ...
            },
            {
                ...
                "next_backup": now
                - timedelta(hours=1),  # Past execution time but disabled
                "enabled": False,
            },
        ]
        ...
        due_schedules = (
            db.query(BackupSchedule)
            .filter(BackupSchedule.enabled == True, BackupSchedule.next_backup_at <= now)
            .all()
        )
...
class TestBackupScheduleLogService:
    ...
    def test_create_schedule_log_success(
        self, db: Session, test_server: Server, test_user: User
    ):
        ...
        created_log = (
            ...
            .filter(BackupScheduleLog.server_id == test_server.id)
            .first()
        )
    ...
    def test_get_logs_by_server_id(
        self, db: Session, test_server: Server, test_user: User
    ):
        ...
        server_logs = (
            db.query(BackupScheduleLog)
            .filter(BackupScheduleLog.server_id == test_server.id)
            .order_by(BackupScheduleLog.created_at)
            .all()
        )
    ...
    def test_log_with_config_changes(
        self, db: Session, test_server: Server, test_user: User
    ):
        ...
        created_log = (
            db.query(BackupScheduleLog)
            .filter(BackupScheduleLog.action == ScheduleAction.updated)
            .first()
        )
    ...
    def test_system_executed_log(self, db: Session, test_server: Server):
        ...
        system_log = (
            db.query(BackupScheduleLog)
            .filter(BackupScheduleLog.executed_by_user_id.is_(None))
            .first()
        )
    ...
    def test_log_chronological_order(
        self, db: Session, test_server: Server, test_user: User
    ):
