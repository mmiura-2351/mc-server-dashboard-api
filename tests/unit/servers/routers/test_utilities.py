...
class TestUtilitiesRouterSimple:
    ...
    async def test_get_supported_versions_error(self, mock_version_manager):
        ...
        mock_version_manager.get_supported_versions = AsyncMock(side_effect=Exception("Service error"))
    ...
    async def test_get_java_compatibility_info_success(self, mock_java_service):
        ...
        mock_java_service.discover_java_installations = AsyncMock(return_value=mock_installations)
        mock_java_service.get_compatibility_matrix.return_value = mock_compatibility_matrix
        mock_java_service.get_supported_minecraft_versions.return_value = mock_supported_versions
    ...
    @pytest.mark.asyncio
    @patch('app.servers.routers.utilities.java_compatibility_service')
    async def test_validate_java_for_minecraft_version_no_compatible_java(self, mock_java_service):
        """Test Java validation when no compatible Java found"""
        from app.servers.routers.utilities import validate_java_for_minecraft_version
        
        mock_java_service.get_java_for_minecraft = AsyncMock(return_value=None)
        mock_java_service.discover_java_installations = AsyncMock(return_value={8: Mock(), 11: Mock()})
        mock_java_service.get_required_java_version.return_value = 17
    ...
    @pytest.mark.asyncio
    @patch('app.servers.routers.utilities.java_compatibility_service')
    async def test_validate_java_for_minecraft_version_incompatible(self, mock_java_service):
        ...
        mock_java_service.get_java_for_minecraft = AsyncMock(return_value=mock_java_info)
        mock_java_service.validate_java_compatibility.return_value = (False, "Version too old")
        mock_java_service.get_required_java_version.return_value = 17
    ...
    async def test_valid_minecraft_version_formats(self):
        ...
        for version in valid_versions:
            try:
                with patch('app.servers.routers.utilities.java_compatibility_service') as mock_service:
    ...
    async def test_get_cache_stats_error(self, mock_cache_manager, admin_user):
        ...
        mock_cache_manager.get_cache_stats = AsyncMock(side_effect=Exception("Cache error"))
    ...
    async def test_cleanup_cache_error(self, mock_cache_manager, admin_user):
        ...
        mock_cache_manager.cleanup_old_cache = AsyncMock(side_effect=Exception("Cleanup error"))
    ...
    async def test_get_java_compatibility_info_error(self, mock_java_service):
        ...
        mock_java_service.discover_java_installations = AsyncMock(side_effect=Exception("Service error"))

        with pytest.raises(HTTPException) as exc_info:
            await get_java_compatibility_info()
        
        assert exc_info.value.status_code == 500
        assert "Failed to get Java compatibility information" in str(exc_info.value.detail)
    ...
    async def test_validate_java_for_minecraft_version_error(self, mock_java_service):
        ...
        mock_java_service.get_java_for_minecraft = AsyncMock(side_effect=Exception("Service error"))
