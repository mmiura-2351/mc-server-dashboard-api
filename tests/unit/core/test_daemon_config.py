...
class TestDaemonConfig:
    ...
    def test_resource_limits_validation(self):
        ...
        # Test valid limits
        limits = DaemonProcessLimits(
            max_memory_mb=1024, max_cpu_percent=50.0, max_open_files=512, max_processes=5
        )
    ...
    def test_configuration_validation_failures(self):
        ...
        config = DaemonConfig(enable_process_persistence=False, enable_auto_recovery=True)
        errors = config.validate_configuration()
        assert len(errors) >= 1
        assert any(
            "Auto recovery requires process persistence" in error for error in errors
        )

        # Test auto recovery without monitoring
        config = DaemonConfig(enable_process_monitoring=False, enable_auto_recovery=True)
        errors = config.validate_configuration()
        assert len(errors) >= 1
        assert any(
            "Auto recovery requires process monitoring" in error for error in errors
        )

        # Test invalid timeout relationships
        config = DaemonConfig(
            process_startup_timeout_seconds=5, monitoring_interval_seconds=10
        )
        errors = config.validate_configuration()
        assert len(errors) >= 1
...
class TestDaemonConfigGlobal:
    ...
    def test_set_and_get_daemon_config(self):
        ...
        custom_config = DaemonConfig(
            daemon_mode=DaemonMode.SUBPROCESS_DAEMON, monitoring_interval_seconds=30
        )
    ...
    def test_validate_daemon_configuration_failure(self):
        ...
        invalid_config = DaemonConfig(
            enable_process_persistence=False, enable_auto_recovery=True
        )
    ...
    def test_pid_directory_from_environment(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            env_vars = {"DAEMON_PID_DIRECTORY": temp_dir}
