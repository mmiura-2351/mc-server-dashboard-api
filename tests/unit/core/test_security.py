...
class TestPathValidator:
    ...
    def test_validate_safe_path_invalid_path(self):
        ...
        assert "Invalid path" in str(
            exc_info.value
        ) or "Path traversal attempt detected" in str(exc_info.value)
    ...
    def test_create_safe_server_directory_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                PathValidator, "sanitize_directory_name", return_value="safe-server-name"
            ):
                result = PathValidator.create_safe_server_directory(
    ...
    def test_create_safe_server_directory_unsafe_name(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                ...
                side_effect=SecurityError("Unsafe name"),
            ):
    ...
    def test_sanitize_directory_name_valid(self):
        ...
        test_cases = [
            ...
            (
                "test@server#",
                "test_server",
            ),  # Invalid chars become underscores, trailing underscore stripped
...
class TestTarExtractor:
    ...
    def test_validate_archive_safety_total_size_too_large(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar.gz") as temp_file:
            ...
            large_size = (
                TarExtractor.MAX_EXTRACTED_SIZE // 2 + 10 * 1024 * 1024
            )  # Just over half
            mock_members = [
    ...
    def test_validate_tar_member_outside_target(self):
        ...
        with patch.object(Path, "resolve") as mock_resolve:
            ...
            assert "Tar member would extract outside target directory" in str(
                exc_info.value
            )
...
class TestFileOperationValidator:
    ...
    def test_validate_server_file_path_unsafe_server_name(self):
        ...
        with patch.object(
            PathValidator, "validate_safe_name", side_effect=SecurityError("Unsafe name")
        ):
            with pytest.raises(SecurityError):
                FileOperationValidator.validate_server_file_path(
    ...
    def test_validate_server_file_path_server_dir_traversal(self):
        ...
        with patch.object(
            PathValidator,
            "validate_safe_path",
            side_effect=SecurityError("Path traversal"),
        ):
            with pytest.raises(SecurityError):
                FileOperationValidator.validate_server_file_path(
                    "test-server", "config.txt", base_dir
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(PathValidator, "validate_safe_path") as mock_validate:
                mock_validate.side_effect = [
                    ...
                    SecurityError(
                        "File outside server directory"
                    ),  # Second call fails (file path)
                ]
