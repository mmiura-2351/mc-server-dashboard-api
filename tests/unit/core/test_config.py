...
class TestSettingsValidators:
    ...
    def test_validate_secret_key_weak_values(self):
        ...
        for weak_key in weak_keys:
            with pytest.raises(ValidationError) as exc_info:
                Settings(SECRET_KEY=weak_key, DATABASE_URL="sqlite:///test.db")
    ...
    def test_validate_secret_key_valid(self):
        ...
        settings = Settings(SECRET_KEY=valid_key, DATABASE_URL="sqlite:///test.db")
    ...
    def test_validate_queue_size_too_small(self):
        ...
        assert "SERVER_LOG_QUEUE_SIZE must be between 100 and 10000" in str(
            exc_info.value
        )
    ...
    def test_validate_java_timeout_too_small(self):
        ...
        assert "JAVA_CHECK_TIMEOUT must be between 1 and 60 seconds" in str(
            exc_info.value
        )
    ...
    def test_validate_db_backoff_too_small(self):
        ...
        assert "DATABASE_RETRY_BACKOFF must be between 0.01 and 5.0 seconds" in str(
            exc_info.value
        )
