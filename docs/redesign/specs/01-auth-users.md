...
- 403 owner 以外は削除不可
- 409 `Organization has active servers. Delete them first.`
...
| メンバー追加 | `member_add` | organization |
| メンバー権限変更 | `member_role_change` | organization |
| メンバー削除 | `member_remove` | organization |
| 招待送信 | `invitation_send` | organization |
| 招待承諾 | `invitation_accept` | organization |
