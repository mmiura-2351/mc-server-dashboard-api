...
| permissions | JSON | NOT NULL | [] | 付与されているパーミッション一覧 |
...
**UNIQUE 制約:** (organization_id, user_id) WHERE deleted_at IS NULL (partial unique index)

**permissions の構造:**
```json
["server.read", "server.start", "server.stop", "backup.create"]
```
...
ロールテンプレートは持たない。メンバーごとにパーミッションを直接付与する。
Organization 作成者は初期状態ですべてのパーミッションを持つ。

**特例: Organization 削除 (`org.delete`)** はパーミッションではなく、`Organization.owner_user_id` のユーザーのみが実行可能な操作として扱う。
...
**認証:** `Organization.owner_user_id` のユーザーのみ
...
2. Organization 内にサーバーが存在する場合は削除不可 (論理削除済みを除く)
3. `deleted_at = now()` で論理削除。カスケード対象:
   - `OrganizationMember` → 論理削除
   - `Group` / `GroupPlayer` → 論理削除
4. 監査ログ記録

**エラー:**
- 400 `confirm must be true`
- 403 `Only the organization owner can delete the organization`
...
```json
{
  "permissions": ["server.read", "server.start", "backup.create"]
}
```
...
**バリデーション:**
- `permissions` は定義済みパーミッション一覧の値のみ使用可能
...
**処理フロー:**
1. 対象ユーザーが `Organization.owner_user_id` の場合は削除不可
2. `deleted_at = now()` で論理削除
3. 監査ログ記録

**エラー:**
- 400 `Cannot remove the organization owner. Transfer ownership first.`
- 403 権限不足

---

### POST /api/v2/organizations/{org_id}/transfer-ownership — オーナー移譲
...
**処理フロー:**
1. `new_owner_user_id` が Organization のアクティブなメンバーであることを確認
2. `Organization.owner_user_id` を `new_owner_user_id` に更新
3. 監査ログ記録

**エラー:**
- 400 `New owner must be an active member of the organization`
...
**Note:** 招待承諾後のメンバーは初期状態でパーミッションを持たない。招待後に `PUT .../members/{user_id}` で権限を付与する。
...
4. OrganizationMember レコードを作成 (`permissions = []` で初期化)
...
| PAT 名 | 1-100 文字 |
| permissions | 定義済みパーミッション一覧の値のみ使用可能 |
...
| オーナー移譲 | `org_transfer_ownership` | organization |
| 招待送信 | `member_invite` | organization |
| メンバー参加 (招待承諾) | `member_join` | organization |
| メンバー権限変更 | `member_permissions_update` | organization |
